...
describe('PaymentsService', () => {
  ...
  describe('createPayment', () => {
    it('should throw BadRequestException if both bookingId and appointmentId are provided', async () => {
      await expect(service.createPayment('u1', { bookingId: 'b1', appointmentId: 'a1', paymentMethod: PaymentMethod.CARD }))
        .rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException if family member not found', async () => {
      mockFamilyRepo.findOne.mockResolvedValue(null);
      await expect(service.createPayment('u1', { bookingId: 'b1', paymentMethod: PaymentMethod.CARD }))
    ...
    describe('with bookingId', () => {
      ...
      it('should throw BadRequestException if booking is cancelled', async () => {
        mockFamilyRepo.findOne.mockResolvedValue({ id: 'f1' });
        mockBookingRepo.findOne.mockResolvedValue({ userId: 'f1', status: BookingStatus.CANCELLED });
        await expect(service.createPayment('u1', { bookingId: 'b1', paymentMethod: PaymentMethod.CARD }))
          .rejects.toThrow(BadRequestException);
      });
    });
        ...
        const manager = {
          getRepository: jest.fn().mockImplementation((entity) => {
            if (entity === Payment) return {
              findOne: jest.fn().mockResolvedValue({ id: 'p1', paymentMethod: PaymentMethod.BANK_TRANSFER, status: PaymentStatus.PENDING_APPROVAL }),
              save: jest.fn().mockResolvedValue({ id: 'p1', status: PaymentStatus.PAID } as any),
            };
            if (entity === Booking) return {
              findOne: jest.fn().mockResolvedValue({ status: BookingStatus.PENDING_PAYMENT, carePlanSnapshot: { name: 'Basic' } }),
              save: jest.fn(),
            };
