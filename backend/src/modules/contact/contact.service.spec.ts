...
describe('ContactService', () => {
  ...
  describe('getAllMessages', () => {
    it('should return messages with total and pending counts', async () => {
      ...
      mockMessageRepo.findAndCount.mockResolvedValue([msgs, 3]);
      mockMessageRepo.count.mockResolvedValue(2);

      const result = await service.getAllMessages(1, 10);
  ...
  describe('getMessage', () => {
    ...
    it('should throw NotFoundException when message not found', async () => {
      ...
      await expect(service.getMessage('missing')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('replyToMessage', () => {
    ...
    it('should persist the reply text and repliedBy', async () => {
      ...
      mockMessageRepo.findOne.mockResolvedValue(msg);
      mockMessageRepo.save.mockImplementation(async (m) => m);
      mockInfoRepo.findOne.mockResolvedValue({
        phonePrimary: '1',
        email: 'a@test.com',
      });
    ...
    it('should throw BadRequestException when message already replied', async () => {
      ...
      await expect(
        service.replyToMessage('1', { reply: 'Hi' }, 'admin1'),
    ...
    it('should throw NotFoundException when message does not exist', async () => {
      mockMessageRepo.findOne.mockResolvedValue(null);
      await expect(
        service.replyToMessage('missing', { reply: 'Hi' }, 'admin'),
