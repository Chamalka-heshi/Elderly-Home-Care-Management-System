...
describe('PatientsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw ConflictException on duplicate NIC', async () => {
      ...
      await expect(service.create('f1', dto)).rejects.toThrow(ConflictException);
    });
  });

  describe('findOneByFamily', () => {
    it('should find patient', async () => {
      mockPatientRepo.findOne.mockResolvedValue({ id: 'p1', familyMemberId: 'f1' });
    ...
    it('should throw NotFoundException if not belong to family', async () => {
      ...
      await expect(service.findOneByFamily('p1', 'f1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    it('should update if authorized', async () => {
      ...
      expect(mockPatientRepo.update).toHaveBeenCalledWith('p1', { fullName: 'Updated' });
    });

    it('should throw ForbiddenException if unauthorized', async () => {
      mockPatientRepo.findOne.mockResolvedValue({ id: 'p1', familyMemberId: 'f2' });
      await expect(service.update('p1', 'f1', {})).rejects.toThrow(ForbiddenException);
    });
  });
