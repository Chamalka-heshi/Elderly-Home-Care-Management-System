...
describe('AppointmentService', () => {
  ...
  describe('createAppointment', () => {
    ...
    it('should throw BadRequestException when slot is fully booked', async () => {
      ...
      await expect(service.createAppointment('u1', dto)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('getMyAppointments', () => {
    ...
    it('should throw NotFoundException when family member not found', async () => {
      ...
      await expect(service.getMyAppointments('u1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('cancelMyAppointment', () => {
    ...
    it('should throw NotFoundException when appointment not found', async () => {
      ...
      await expect(
        service.cancelMyAppointment('u1', 'missing'),
  ...
  describe('getDoctorAppointments', () => {
    ...
    it('should throw NotFoundException when doctor profile not found', async () => {
      ...
      await expect(service.getDoctorAppointments('u1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateAppointmentStatusByDoctor', () => {
    ...
    it('should update appointment status when doctor owns slot', async () => {
      ...
      const result = await service.updateAppointmentStatusByDoctor(
        'u1',
        'a1',
        dto,
      );
    ...
    it('should throw NotFoundException when appointment not found', async () => {
      ...
      mockApptRepo.findOne.mockResolvedValue(null);
      await expect(
        service.updateAppointmentStatusByDoctor('u1', 'missing', dto),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException when appointment belongs to another doctor', async () => {
      ...
      mockApptRepo.findOne.mockResolvedValue({
        id: 'a1',
        slot: { doctorId: 'OTHER_DOCTOR' },
      });
      await expect(
        service.updateAppointmentStatusByDoctor('u1', 'a1', dto),
  ...
  describe('adminUpdateStatus', () => {
    it('should update appointment status as admin', async () => {
      ...
      const result = await service.adminUpdateStatus('a1', {
        status: AppointmentStatus.CANCELLED,
      });
    ...
    it('should throw NotFoundException when appointment not found', async () => {
      mockApptRepo.findOne.mockResolvedValue(null);
      await expect(
        service.adminUpdateStatus('missing', {
          status: AppointmentStatus.CANCELLED,
        }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should persist notes when provided', async () => {
      ...
      await service.adminUpdateStatus('a1', {
        status: AppointmentStatus.COMPLETED,
        notes: 'OK',
      });
      expect(mockApptRepo.save).toHaveBeenCalledWith(
  ...
  describe('adminDelete', () => {
    ...
    it('should throw NotFoundException when appointment not found', async () => {
      mockApptRepo.findOne.mockResolvedValue(null);
      await expect(service.adminDelete('missing')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
