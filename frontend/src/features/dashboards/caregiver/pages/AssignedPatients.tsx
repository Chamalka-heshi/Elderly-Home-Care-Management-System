...
const AssignedPatients: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Info banner — Phase 1 note */}
      <div className="flex items-start gap-3 rounded-2xl border border-sky-200 bg-sky-50 px-5 py-4 text-sm text-sky-800">
        <span className="mt-0.5 shrink-0 text-lg">ℹ️</span>
        <p>
          <span className="font-bold">All registered patients</span> are currently visible to caregivers.
          Once the payments module is active, only patients on an active payment plan will appear here.
        </p>
      </div>
      ...
      <TableShell
        ...
        subtitle={loading ? "Loading…" : `${filtered.length} patient${filtered.length !== 1 ? "s" : ""} found.`}
      >
        {loading ? (
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((p) => (
                  <tr key={p.id} className="transition hover:bg-slate-50/60">
                    ...
                    <td className="max-w-[160px] truncate px-4 py-3 text-slate-600">{p.chronicConditions ?? "—"}</td>
                    <td className="max-w-[120px] truncate px-4 py-3 text-slate-600">{p.allergies ?? "—"}</td>
                    <td className="px-4 py-3">
                      <span className={[
                        ...
                        <span className={["h-1.5 w-1.5 rounded-full", p.isActive ? "bg-emerald-500" : "bg-slate-400"].join(" ")} />
