...
const subtitleMap: Record<MenuLabel, string> = {
  "Dashboard":          "Monitor, manage, and keep everything running smoothly.",
  "Assigned Patients":  "View and manage your assigned patients.",
  "Care Notes":         "Log and review daily patient care notes.",
  "Vital Records":      "Record and track patient vital signs.",
  "Medication Updates": "Monitor and update patient medication status.",
  "Care Schedule":      "View your upcoming care shifts and tasks.",
...
const Topbar: React.FC<Props> = ({ activeMenu, onToggleSidebar, onProfileClick }) => {
  ...
  const displayName = user?.full_name ?? "Caregiver User";
  const displayRole =
    user?.role === "caregiver"  ? "System Administrator" :
    user?.role === "family"     ? "Family Member"   :
    user?.role === "doctor"     ? "Doctor"           :
    user?.role === "admin"      ? "Administrator"    : "System Administrator";
