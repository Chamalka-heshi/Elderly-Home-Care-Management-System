...
const DoctorProfile: React.FC<Props> = ({ onBack }) => {
  ...
  // ── Editable profile fields (always start empty — filled by API) ──────
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Sync AuthContext with latest server data
        setUser(freshUser);

        // Sync localStorage so page refreshes are consistent
        localStorage.setItem("user", JSON.stringify(freshUser));

        // Safely read the nested doctor profile object
        const doctorProfile = (freshUser as any)?.profile ?? {};

        // Pre-fill ALL editable fields from the fresh API response
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      setProfLoading(true);
      const updatedUser = await updateDoctorProfile({
        fullName,
        contactNumber,
        specialization,
        licenseNumber,
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    // Full validation — matches AdminProfile standard
    if (!currentPw) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 md:px-6">
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1 rounded-2xl border border-white/30 bg-white/60 p-1 shadow-sm backdrop-blur-xl">
            {tabs.map(({ key, label, icon: Icon }) => (
              <button
                key={key}
                onClick={() => setTab(key)}  // ✅ type-safe — key is already TabKey
                type="button"
                className={[
        ...
        {tab === "danger" && (
          <DangerZoneTab
            deactivateNote="Temporarily deactivate your doctor access. An admin can reactivate it."
            deleteNote="Permanently delete your doctor account and all associated data. This cannot be undone."
            footerNote="Destructive account actions require confirmation from an administrator."
            footerIcon={IconShield}
            onDeactivate={() =>
              addToast("error", "Contact Admin to deactivate your account.")
            }
            onDelete={() =>
              addToast("error", "Contact Admin to delete your account.")
            }
          />
        )}
      </main>
