...
const BStatCard: React.FC<{
  title: string; value: string; sub?: string; subTone?: Tone;
  icon: React.FC<{ className?: string }>; color: string;
}> = ({ title, value, sub, subTone = "default", icon: Icon, color }) => (
  <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
    <div className="flex items-start justify-between">
      <div className="min-w-0">
        <p className="text-xs font-semibold text-slate-500">{title}</p>
        <p className="mt-1 text-xl font-bold text-slate-900 truncate">{value}</p>
        {sub && <p className={`mt-0.5 text-[11px] font-medium ${TONE_TEXT[subTone]}`}>{sub}</p>}
      </div>
...
// ──────────────────────────────────────────────────────────────────────────
// TAB 1 — Backups (stat summary + history table, one continuous view)
// ──────────────────────────────────────────────────────────────────────────
...
const BackupsTab: React.FC<{
  ...
  stats, statsLoading, statsError, onRetryStats,
  records, total, page, pages, search, historyLoading, isCreating, isRestoring,
  onSearch, onPage, onRefresh, onCreateNow, onDelete, onView, onRestore,
}) => {
  const latestStatus = stats?.latest?.status;
  const latestIcon =
    latestStatus === "success" ? IconCheckCircle :
    latestStatus === "failed"  ? IconAlertCircle :
    IconClock;
  const latestColor =
    latestStatus === "success" ? "bg-emerald-50 text-emerald-700" :
    latestStatus === "failed"  ? "bg-red-50 text-red-600" :
    latestStatus ? "bg-amber-50 text-amber-700" :
    "bg-blue-50 text-blue-700";
  const latestTone: Tone =
    latestStatus === "success" ? "success" :
    latestStatus === "failed"  ? "danger" :
    latestStatus ? "warning" : "default";
  ...
  return (
    <div className="space-y-6">
      ...
      {statsLoading ? (
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-[74px] animate-pulse rounded-2xl border border-slate-100 bg-slate-50" />
          ))}
        </div>
      ) : statsError || !stats ? (
        <div className="flex items-center justify-between rounded-2xl border border-red-100 bg-red-50/60 px-4 py-3">
          <span className="flex items-center gap-2 text-sm font-medium text-red-700">
            <IconAlertCircle className="h-4 w-4" /> Couldn't load backup summary.
          </span>
          <button
            onClick={onRetryStats}
            className="flex items-center gap-1.5 rounded-xl border border-red-200 bg-white px-3 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-50"
          >
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <BStatCard
            ...
            sub={stats.latest ? `${fmtTime(stats.latest.createdAt)} · ${cap(latestStatus ?? "")}` : "No backups yet"}
            subTone={latestTone}
            icon={latestIcon}
            color={latestColor}
          />
          ...
          <BStatCard
            ...
            sub={`${stats.success} successful backup${stats.success !== 1 ? "s" : ""}`}
            icon={IconStorage}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 max-w-xs">
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm text-slate-800 outline-none transition focus:border-emerald-300 focus:ring-4 focus:ring-emerald-500/10"
          />
      ...
      <p className="text-xs text-slate-500 -mt-2">{total} backup{total !== 1 ? "s" : ""} found</p>
...
const inputCls =
  "w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 outline-none transition focus:border-emerald-300 focus:ring-4 focus:ring-emerald-500/10 disabled:opacity-50";
...
type Tab = "backups" | "settings" | "logs";

const TABS: { id: Tab; label: string; icon: React.FC<{ className?: string }> }[] = [
  { id: "backups",  label: "Backups",     icon: IconDatabase },
...
const BackupRestore: React.FC = () => {
  ...
  // Initial load — stats and history both live on the default "backups" tab
  useEffect(() => { loadStats(); }, [loadStats]);
  useEffect(() => {
    if (activeTab === "backups") loadHistory(1, "");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-bold text-slate-900">Backup &amp; Recovery</h2>
          <p className="mt-1 text-sm text-slate-600">Automated, encrypted AWS S3 snapshots for ECMS data security.</p>
        </div>
        <button
          onClick={handleCreateNow}
          disabled={isCreating}
          className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-600/25 transition hover:-translate-y-0.5 hover:bg-emerald-700 disabled:opacity-60 disabled:hover:translate-y-0"
        >
