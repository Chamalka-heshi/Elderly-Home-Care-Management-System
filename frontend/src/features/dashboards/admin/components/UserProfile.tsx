...
const UserProfile: React.FC<Props> = ({ open, onClose }) => {
  ...
  // Local editable state (mirrors auth user)
      ...
      <aside className={[
        ...
        <div className="relative border-b border-white/30 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-base font-bold text-slate-900">{user?.full_name ?? "Admin User"}</p>
              ...
              <div className="rounded-2xl border border-white/30 bg-white/60 px-4 py-4 shadow-sm">
                ...
                <div className="space-y-2 text-xs text-slate-500">
                  <div className="flex justify-between">
                    <span>User ID</span>
                    <code className="rounded-lg bg-slate-100 px-2 py-0.5 text-slate-700 font-mono">
                      {user?.id?.slice(0, 16) ?? "—"}…
                    </code>
                  </div>
                  <div className="flex justify-between">
            ...
            <form onSubmit={handleChangePassword} className="space-y-5">
              <div className="flex items-start gap-3 rounded-2xl border border-amber-200/60 bg-amber-50/60 px-4 py-3">
                ...
                <p className="text-xs text-amber-800">
                  Choose a strong password that's at least 8 characters and unique to this account.
                </p>
              </div>
        ...
        <div className="border-t border-white/30 px-6 py-4">
          <p className="text-center text-[11px] text-slate-400">
            Care Home ECMS · Admin Panel · Secure session
          </p>
        </div>
      </aside>
    </>
