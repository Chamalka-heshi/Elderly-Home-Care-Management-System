...
// ── demo config (replace with real API calls) ─────────────────────────────────
const DEMO_EMAILS = [
  "admin@carehome.com",
  "doctor@carehome.com",
...
export default function ForgotPasswordCard({ onGoLogin }: Props) {
  ...
  const onSubmitOtp = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError("");
    if (otp.trim() !== DEMO_OTP) { setError(`Invalid OTP. (Demo hint: ${DEMO_OTP})`); return; }
    setStep("newpassword");
  };
  ...
  return (
    <div className="grid items-stretch lg:grid-cols-2">
      ...
      {/* LEFT IMAGE — identical to LoginCard / SignupCard */}
